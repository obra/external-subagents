...
describe('start command', () => {
  ...
  it('runs codex exec, stores registry metadata, and logs messages', async () => {
    ...
    expect(registry[fixture.thread_id].permissions).toBe('read-only');
  ...
  it('applies persona prompt, skills, and model mapping', async () => {
    ...
    const callOptions = vi.mocked(runExec).mock.calls[0]?.[0];
    expect(callOptions?.permissions).toBe('read-only');
    expect(callOptions?.transformPrompt).toBeTypeOf('function');
  ...
  it('defaults to codex backend when not specified', async () => {
    const root = await mkdtemp(path.join(os.tmpdir(), 'codex-subagent-default-backend-'));
    const codexRoot = path.join(root, '.codex-subagent');
    ...
    await startCommand({
      ...
      role: 'researcher',
      permissions: 'workspace-write',
      promptBody: 'Default backend test',
