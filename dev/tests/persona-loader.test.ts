...
import { loadPersonaRuntime, mapModelAliasToPermissions } from '../src/lib/personas.ts';
...
describe('persona loader', () => {
  ...
  it('maps model aliases to permissions', () => {
    expect(mapModelAliasToPermissions('haiku').permissions).toBe('read-only');
    expect(mapModelAliasToPermissions('sonnet').permissions).toBe('workspace-write');
    expect(mapModelAliasToPermissions('inherit').permissions).toBeUndefined();
  });
});
